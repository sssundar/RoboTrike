...
; SerialIO_EH  
;   flow control for determining which specific event handler to use
; LoopTillSerialPut    
;   helper that loops calling serialputchar till carry flag reset.
; LoopTillStringPut
;   helper that calls LoopTillSerialPut on every char in a string.
...
;   helper functions.
;  12/31/2014 SSundaresh removed timer nesting from SerialEH for UI board
;                        added LoopTillStringPut for UI board.
...
; take ES:BX as the string to transmit. Loop till 
; reach ASCII_NULL in string.
; change no registers but flags
; Last Revised: 12/31/2014 SSundaresh created
LoopTillStringPut PROC NEAR
        PUBLIC LoopTillStringPut
    PUSHA
